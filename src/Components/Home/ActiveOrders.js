                    ...
                    <Grid item lg={3} md={4} sm={6} xs={12}>
                        <Paper elevation={3} className={classes.itemRoot}>
                            <Grid container direction='column' >
                                <Typography align='left' className={classes.itemTitle}>{order.cartItems.length > 1 ? (order.cartItems[0].productName + ' + ' + (order.cartItems.length - 1) + ' more item(s)' ): order.cartItems[0].productName}</Typography>
                                <Typography align='left' className={classes.shopName}>{order.shopName}</Typography>
                                <Typography align='left' className={classes.itemOrderedDate}>Ordered on : {getFormattedDate(order.date)}</Typography>
                                <Typography align='left' className={classes.itemOrderedStatus}>Order Status : {order.delStatus}</Typography>
                                <Typography align='left' className={classes.itemExpectedDelivery}>Expected Delivery: {getFormattedDate(order.expectedDate)}</Typography>
                                {order.delStatus==='Out for Delivery'&&
                                    <Grid container direction='column'>
                                        <Typography align='left' className={classes.itemDeliveryDetails}>Delivery Details</Typography>
                                ...
                                <Button variant='outlined' className={classes.trackBtn} onClick={() => handleTrackClick(order)} color='secondary' fullWidth endIcon={<LocationOn></LocationOn>}>Track Order</Button>
                            </Grid>
                        </Paper>
                    </Grid>
                ))}
